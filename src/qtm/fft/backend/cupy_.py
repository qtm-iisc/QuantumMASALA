...
class CuPyFFTWrapper(FFTBackend):
    ...
    def __init__(self, arr: tuple[int, ...], axes: tuple[int, ...]):
        ...
        self.fft_plan = get_fft_plan(self._inp_fwd, axes=self.axes, value_type="C2C")

    @classmethod
    def allocate_array(cls, shape: int | Sequence[int], dtype: str) -> cp.ndarray:
        return cp.empty(shape, dtype=dtype, order="C")

    def fft(self) -> cp.ndarray:
        return fftn(
            self._inp_fwd, axes=self.axes, plan=self.fft_plan, overwrite_x=False
        )
