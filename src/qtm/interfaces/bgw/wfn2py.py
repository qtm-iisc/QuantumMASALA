...
def wfn2py(filename="../test/bgw/WFN.h5", verbose=False):
    ...
    isort_py = np.lexsort([grho.g_cryst[2], grho.g_cryst[1], grho.g_cryst[0]])

    assert np.allclose(
        components[isort_bgw], grho.g_cryst.T[isort_py]
    ), "Mismatch: G-space components"

    # Generating a mapping between 'grho.g_cryst' and 'gspace["components"]'
    idx_grho_bgw2pw = np.empty(grho.size_g, dtype="i4")
    idx_grho_bgw2pw[isort_py] = np.arange(grho.size_g, dtype="i4")[isort_bgw]

    assert np.allclose(
        grho.g_cryst.T, components[idx_grho_bgw2pw]
    ...
    # GkSpace
    # =========
    ecutwfn = np.array(kpoints["ecutwfc"])
    wfn_gspc = GSpace(
        recilat=recilat, ecut=4 * ecutwfn * RYDBERG, grid_shape=grho.grid_shape
    )
    l_gk = [GkSpace(wfn_gspc, k_cryst) for k_cryst in kpts.cryst]

    assert np.allclose(
        [gk.size_g for gk in l_gk], np.array(kpoints["ngk"])
    ), "Mismatch: ngk & [gk.size_g]"
    ...
    for i, gk in enumerate(l_gk):
        ...
        idxbgw2pw = np.empty(gk.size_g, dtype="i4")
        idxbgw2pw[isort_py] = np.arange(gk.size_g, dtype="i4")[isort_bgw]
