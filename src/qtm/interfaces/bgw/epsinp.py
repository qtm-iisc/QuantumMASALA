...
class Epsinp:
    ...
    You can generate this list with ``kgrid.x``: just set the shifts to zero and use same grid numbers as for ``WFN``. 
    Then replace the zero vector with q0.
    """

    frequency_dependence_method: int = None
    r"""Full frequency dependence method for the polarizability, if ``frequency_dependence`` == 2:
    ...
    r"""Broadening parameter for each val->cond transition. It should correspond to the energy resolution due to k-point sampling, or a number as small as possible if you have a molecule. The default value is 0.1 for method 0 and 1, and 0.25 for method 2.
    """
    init_frequency: float = None
    r"""Lower bound for the linear frequency grid.
    """
    frequency_low_cutoff: float = None
    r"""Upper bound for the linear frequency grid. For methods 0 and 1, it is also the lower bound for the non-uniform frequency grid. Should be larger than the maximum transition, i.e., the energy difference between the highest conduction band and the lowest valence band. The default is 200 eV for method 0 and 1, and 10 eV for method 2. For method 2, you can increase ``frequency_low_cutoff`` if you wish to use the Sigma code and look into QP states deep in occupied manifold or high in the unoccupied manifold.
    """
    delta_frequency: float = None
    r"""Frequency step for full-frequency integration for the linear grid Should be converged (the smaller, the better). For molecules, ``delta_frequency`` should be the same as broadening. Defaults to the value of broadening.
    """
    number_imaginary_freqs: int = None
    r"""Number of frequencies on the imaginary axis for method 2.
    """
    frequency_high_cutoff: float = None
    r"""Upper limit of the non-uniform frequency grid. Defaults to 4*``frequency_low_cutoff``
    """
    delta_frequency_step: float = None
    r"""Increase in the frequency step for the non-uniform frequency grid.
    """
    delta_sfrequency: float = None
    r"""Frequency step for the linear grid for the spectral function method. Defaults to ``delta_frequency``.
    """
    delta_sfrequency_step: float = None
    r"""Increase in frequency step for the non-uniform grid for the spectral function method. Defaults to ``delta_frequency_step``
    """
    sfrequency_low_cutoff: float = None
    r"""Upper bound for the linear grid for the spectral function method and lower bound for the non-uniform frequency grid. Defaults to ``frequency_low_cutoff``
    """
    sfrequency_high_cutoff: float = None
    r"""Upper limit of the non-uniform frequency grid for the spectral function method. Defaults to ``frequency_low_cutoff``
    """
    r"""### Static subspace approximation
    Input parameters controlling the full-frequency static subspace approximation method. The method speeds up full-frequency calculations by expanding the frequency-dependent part of the polarizability in the subspace basis formed by the lowest eigenvectors of the static polarizability (see `Phys. Rev. B 99, 125128, 2019 <https://journals.aps.org/prb/abstract/10.1103/PhysRevB.99.125128>`__). The method is implemented for the case of full-frequency contour-deformation formalism, i.e., ``frequency_dependence``==2 and ``frequency_dependence_method``==2.
    """
    chi_eigenvalue_cutoff: float = None
    r"""Activate the static subspace approximation and define the screening threshold for the eigenvalues of the static polarizability.
    """
    nbasis_subspace: int = None
    r"""Define a maximum fixed number of eigenvectors to be used (recommended). Set for example to 25% of the number of G-vectors employed for the expansion of chi0.
    """
    evs: float = None
    """"""
    ev0: float = None
    ...
    r"""This specifies the radius of for spherical truncation, in Bohr, so that the Coulomb potential $v(r)$ is zero for $r$ larger than these values. This flag is to be used together with ``spherical_truncation`` (http://manual.berkeleygw.org/3.0/epsilon-keywords/#spherical_truncation).
    """

    r"""Misc. parameters
    """
    epsilon_cutoff: float = None
    r"""Energy cutoff for the dielectric matrix, in Ry. The dielectric matrix $\varepsilon_{GG'}$ will contain all G-vectors with kinetic energy $|q+G|^2$ up to this cutoff.
    """
    number_bands: int = None
    r"""Total number of bands (valence+conduction) to sum over. Defaults to the number of bands in the WFN file minus 1.
    """
    frequency_dependence: int = None
    r"""This flags specifies the frequency dependence of the inverse dielectric matrix:
    ...
    r"""Plasma frequency (eV) needed for the contour-deformation method (i.e., ``frequency_dependence==2``). The exact value is unimportant, especially if you have enough imaginary frequency points. We recommend you keep this value fixed at 2 Ry.
    """
    imaginary_frequency: float = None
    r"""For ``frequency_dependence==3``, the value of the purely imaginary frequency, in eV:
    """
    full_chi_conv_log: int = None
    r"""Logging convergence of the head & tail of polarizability matrix with respect to conduction bands:
    ...
    r"""Number of pools for distribution of valence bands The default is chosen to minimize memory in calculation
    ...
    """
    qgrid: List[int] = None
    r"""Q-grid for the epsmat file. Defaults to the WFN k-grid.
    """

    # Options:
    write_subspace_epsinv: bool = False
    r"""Write frequency-dependent epsilon matrices in `eps0mat[.h5]` and `epsmat[.h5]` files using the subspace basis instead of the full G-vector basis (recommended). This flag need to be specified to use the full-frequency static subspace approximation in sigma.
    """
    subspace_dont_keep_full_eps_omega0: bool = False
    r"""Discharge the output of the full static epsilon matrix in the `eps0mat[.h5]` and `epsmat[.h5]` files.
    """
    subspace_use_elpa: bool = False
    r"""If ELPA library is linked, specify to use ELPA for the diagonalization of the static polarizability.
    """
    dont_keep_fix_buffers: bool = False
    r"""Using `comm_nonblocking_cyclic` force the algorithm to use buffers for communication with variable size, mainly used for debugging.
    """
    sub_collective_eigen_redistr: bool = False
    r"""Replicate eigenvectors using collective operation in the basis transformation step, performs in general worse than point to point, mainly used for debugging.
    """
    r"""Scissors operator
    ...
    r"""Truncate the Coulomb potential based on the Wigner-Seitz cell. This is the recommended truncation for 0D systems.
    """
    cell_wire_truncation: bool = False
    r"""Truncation scheme for 1D systems, such as carbon nanotubes. The z direction is assumed to be periodic, and x and y confined.
    """
    cell_slab_truncation: bool = False
    r"""Truncation scheme for 2D systems, such as graphene or monolayer MoS2. The z direction is assumed to be confined, and x and y periodic.
    """
    spherical_truncation: bool = False
    r"""Truncate the Coulomb potential based on an analytical scheme. This is ok for quasi-spherical systems, such as CH4 molecule or C60, but the `cell_box_truncation <http://manual.berkeleygw.org/3.0/epsilon-keywords/#cell_box_truncation>`_ is the most general and recommended scheme. When using spherical truncation, you must also specify the radius for the truncation in `spherical_truncation <http://manual.berkeleygw.org/3.0/epsilon-keywords/#spherical_truncation>`_.
    """
    use_wfn_hdf5: bool = False
    r"""Read input wavefunction files in HDF5 format: ``WFN.h5`` instead of ``WFN`` and, if required, ``WFNq.h5`` instead of ``WFNq``.
    """
    comm_nonblocking_cyclic: bool = False
    r"""Within `gcomm_matrix`, employs a non-blocking cyclic communication scheme overlapping computation and communication in the evaluation of the polarizability (drastically reduce the time spent in communication for large runs, but require more memory).
    """
    ...
    r"""Read from traditional simple binary format for epsmat/eps0mat instead of HDF5 file format. Relevant only if code is compiled with HDF5 support.
    """
    eqp_corrections: bool = False
    r"""Set this to use eigenvalues in eqp.dat and eqp_q.dat If not set, these files will be ignored.
    """
    write_vcoul: bool = False
    r"""Write the bare Coulomb potential v(q+G) to file
    """
    gcomm_matrix: bool = False
    r"""Matrix Element Communication Method (Chi Sum Comm). Default is `gcomm_matrix <http://manual.berkeleygw.org/3.0/epsilon-keywords/#gcomm_matrix>`_ which is good if nk,nc,nv>nmtx,nfreq. If nk,nc,nv<nfreq,nmtx (nk,nv<nfreq since nc~nmtx), use gcomm_elements. Only `gcomm_elements <http://manual.berkeleygw.org/3.0/epsilon-keywords/#gcomm_elements>`_ is supported with the spectral method.
    """
    ...
    r"""Replace unfolded BZ with full BZ
    """
    fullbz_write: bool = False
    r"""Write unfolded BZ and full BZ to files
    """
    degeneracy_check_override: bool = False
    r"""The requested number of bands cannot break degenerate subspace Use the following keyword to suppress this check Note that you must still provide one more band in wavefunction file in order to assess degeneracy
    """
    no_min_fftgrid: bool = False
    r"""Instead of using the RHO FFT box to perform convolutions, we automatically determine (and use) the smallest box that is compatible with your epsilon cutoff. This also reduces the amount of memory needed for the FFTs. Although this optimization is safe, you can disable it by uncommenting the following line:
    """
    restart: bool = False
    r"""Use this flag if you would like to restart your Epsilon calculation instead of starting it from scratch. Note that we can only reuse q-points that were fully calculated. This flag is ignored unless you are running the code with HDF5.
    """
    subsample: bool = False
    r"""Use this option to generate an eps0mat file suitable for subsampled Sigma calculations. The only thing this flag does is to allow a calculation with more than one q→0 points without complaining.
    """
