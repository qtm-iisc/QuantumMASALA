...
def minimal_grid_shape(recilat: ReciLattice, ecut: float) -> tuple[int, int, int]:
    ...
    if not isinstance(recilat, ReciLattice):
        raise ValueError(
            f"'recilat' must be a '{ReciLattice}' instance. " f"got {type(recilat)}"
        )
    if ecut <= 0:
        raise ValueError("'ecut' must be a positive number. " f"got {ecut}")
...
def check_grid_shape(
    recilat: ReciLattice, ecut: float, grid_shape: tuple[int, int, int]
):
...
class GSpace(GSpaceBase):
    def __init__(
        ...
        if not isinstance(recilat, ReciLattice):
            raise ValueError(
                f"'recilat' must be a '{ReciLattice}'. " f"got {type(recilat)}"
            )
        ...
        g_cryst = np.array(
            np.meshgrid(*xi, indexing="ij"), like=recilat.primvec
        ).reshape(3, -1)
        ...
        if ke_max <= self.ecut:
            raise ValueError(
                "'ecut' value too large for given 'grid_shape'. "
        ...
        tpiba = recilat.tpiba
        isort = np.argsort(np.around(g_2[icut] / tpiba, ROUND_PREC), kind="stable")
        icut = icut[isort]
