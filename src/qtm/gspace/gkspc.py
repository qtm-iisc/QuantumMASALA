...
class GkSpace(GSpaceBase):
    ...
    def __init__(self, gspc: GSpace, k_cryst: tuple[float, float, float]):
        if not isinstance(gspc, GSpace):
            raise TypeError("'gspc' must be a 'GSpace' instance. "
                            f"got {type(gspc)}")
        self.gspc = gspc
        self.ecutwfn = self.gspc.ecut / 4

        self.k_cryst = tuple(k_cryst)
        gk_cryst = self.gspc.g_cryst.copy().astype('f8')
        for ipol in range(3):
            gk_cryst[ipol] += self.k_cryst[ipol]
        gk_norm2 = self.gspc.recilat.norm2(gk_cryst)
        self.idxgk = np.nonzero(gk_norm2 <= 2 * self.ecutwfn)[0]
        print(self.idxgk.shape)
        super().__init__(self.gspc.recilat, self.gspc.grid_shape,
                         self.gspc.g_cryst[(slice(None), self.idxgk)],
                         )
    ...
    def __eq__(self, other) -> bool:
        if other is self:
            return True
        if not isinstance(other, GkSpace):
            return False
