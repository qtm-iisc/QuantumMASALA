...
class KList:
    def __init__(
        ...
        if not isinstance(recilat, ReciLattice):
            raise TypeError(
                f"'recilat' must be a {ReciLattice} instance. "
                f"got '{type(recilat)}'."
            )
        ...
        if k_coords.ndim != 2:
            raise ValueError(value_mismatch_msg("k_coords.ndim", k_coords.ndim, 2))
    ...
    def __getitem__(self, item) -> KList | tuple[tuple[float, float, float], float]:
    ...
    def scatter(self, n_kgrp: int, i_kgrp: int) -> KList:
        if not isinstance(n_kgrp, int) or n_kgrp <= 0:
            raise TypeError(type_mismatch_msg("n_kgrp", n_kgrp, "a positive integer"))
...
def gen_monkhorst_pack_grid(
    ...
    if (
        not all(isinstance(ni, int) and ni > 0 for ni in grid_shape)
        or len(grid_shape) != 3
    ):
        raise TypeError(
    ...
    if not all(isinstance(si, bool) for si in shifts) or len(shifts) != 3:
        raise TypeError(type_mismatch_msg("shifts", shifts, "three booleans"))
