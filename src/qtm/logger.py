...
LOG_FORMAT: str = (
    "%(asctime)s - proc {MPI_WORLD_RANK}/{MPI_WORLD_SIZE} - "
...
class QTMTimer:
    ...
    def _find_timer(self, label: str) -> int | None:
        ...
        if not isinstance(label, str):
            raise TypeError(
                "'label' must be a string. " f"got {label} (type {type(label)})"
            )
    ...
    def _start_timer(self, label: str) -> None:
        ...
        if itimer is None:
            self.l_timer[self.numtimer] = (label, 0, 0.0, False, 0.0)
    ...
    def __str__(self) -> str:
        ...
        if self.numtimer > 0:
            for count in self.l_timer[: self.numtimer]:
                out += (
                    f"|{count['label']:^30}|{count['call']:8d}|"
                    f"{count['time']:8.2f}|"
...
def qtmlogger_set_filehandle(
    file_dir: str, logging_level: int = logging.INFO, capture_warnings: bool = True
):
    ...
    if logger.hasHandlers():
        warnings.warn(
            f"logger '{LOGGER_NAME}' has handlers already initialized. "
    ...
    if logfile_path.is_file():
        ...
        if MPI_WORLD_RANK == 0:
            ...
            warnings.warn(
                f"log file '{file_dir}' already exists. " f"Renaming it to '{new_dir}'"
            )
