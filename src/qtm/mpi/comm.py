...
class QTMComm:
    ...
    def __init__(
        ...
        if not is_null:
            if self.mpi4py_installed and not isinstance(comm, Intracomm):
                raise TypeError(
                    "'comm' must be a MPI Intracommunicator. " f"got {type(comm)}."
                )
        else:
            comm = None
        ...
        if parent_comm is not None:
            ...
            if parent_comm.is_null:
                raise ValueError(
                    "'parent_comm' is a null communicator i.e "
        ...
        if not self.is_null and parent_comm is not None:
            if self.mpi4py_installed:
                ...
                if Group.Compare(comm_group, grp_intersection) not in [IDENT, SIMILAR]:
                    raise ValueError(
                        "'comm' is not a subgroup of group associated to"
                        "'parent_comm'."
                    )
        ...
        if not isinstance(sync_with_parent, bool):
            raise ValueError(
                "'sync_with_parent' must be a boolean. " f"got {type(sync_with_parent)}"
            )
    ...
    def __getattribute__(self, item):
        ...
        if not isinstance(out, MethodType):
            ...
            raise AttributeError(
                "'CommMod' instance is a null communicator. " "All methods are disabled"
            )
        return out
    ...
    def Incl(
        self, ranks: Sequence[int], sync_with_parent: bool = True
    ) -> QTMComm:  # noqa: N802
        ...
        if self.size == 1:
            ...
            return self
        comm_out = self.comm.Create_group(self.comm.Get_group().Incl(ranks))
        return QTMComm(comm_out, self, sync_with_parent)

    def Root(self):
        return QTMComm(
            self.comm_self if self.rank == 0 else self.comm_null,
            self,
            sync_with_parent=True,
        )

    def Split(
        self, color: int, key: int, sync_with_parent: bool = False
    ) -> QTMComm:  # noqa: N802
    ...
    def Scatter(
        self, sendbuf: BufSpec | None, recvbuf: InPlace | BufSpec, root: int = 0
    ):
        if self.mpi4py_installed:
    ...
    def Scatterv(
        self, sendbuf: BufSpecV | None, recvbuf: InPlace | BufSpec, root: int = 0
    ):
        if self.mpi4py_installed:
    ...
    def Gather(
        self, sendbuf: InPlace | BufSpec, recvbuf: BufSpec | None, root: int = 0
    ):
        if self.mpi4py_installed:
    ...
    def Gatherv(
        self, sendbuf: InPlace | BufSpec, recvbuf: BufSpecV | None, root: int = 0
    ):
        if self.mpi4py_installed:
    ...
    def Allgather(self, sendbuf: InPlace | BufSpec, recvbuf: BufSpec):  # noqa: N802
        """Alias of `mpi4py.MPI.Comm.Allgather`"""
        if self.mpi4py_installed:
    ...
    def Allgatherv(self, sendbuf: InPlace | BufSpec, recvbuf: BufSpecV):  # noqa: N802
    ...
    def Allreduce(
        self, sendbuf: InPlace | BufSpec, recvbuf: BufSpec, op: Op = SUM
    ):  # noqa: N802
        """Alias of `mpi4py.MPI.Comm.Allreduce`"""
        if self.mpi4py_installed:
...
def split_comm_pwgrp(comm: QTMComm, pwgrp_size: int = 1):
    ...
    if not isinstance(pwgrp_size, int) or pwgrp_size <= 0:
        raise ValueError(
            "'pwgrp_size' must be a positive integer. "
            f"got {pwgrp_size} (type {type(pwgrp_size)})."
        )

    if comm_size % pwgrp_size != 0:
        raise ValueError(
            "'pwgrp_size' must evenly divide 'comm's "
