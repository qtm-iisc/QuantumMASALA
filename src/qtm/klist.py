...
class KList:
    def __init__(self, recilat: ReciLattice, cryst: np.ndarray, weights: np.ndarray):
        self.recilat = recilat
        if cryst.shape[0] != weights.shape[0]:
            raise ValueError(
                "length of 'cryst' and 'weights' do not match. "
        ...
        if cryst.ndim != 2 or cryst.shape[1] != 3:
            raise ValueError(
                "'cryst' must be a 2D numpy array representing"
    ...
    def __getitem__(
        self, item
    ) -> Union[KList, tuple[tuple[float, float, float], float]]:
    ...
    def gamma(cls, crystal: Crystal):
        return cls(
            crystal.recilat, np.zeros((1, 3), dtype="f8"), np.ones(1, dtype="f8")
        )
...
def klist_distribute(
    kpts: KList, round_robin: bool = False, return_indices: bool = True
    ...
    if numkpts < pwcomm.numkgrp:
        raise ValueError(
            f"number of k-groups larger than total number of input k-points.\n"
    ...
    if round_robin:
        ...
        start = (numkpts // numkgrp) * idxkgrp + min(idxkgrp, numkpts % numkgrp)
        stop = (numkpts // numkgrp) * (idxkgrp + 1) + min(
            idxkgrp + 1, numkpts % numkgrp
        )
        sl = slice(start, stop)
