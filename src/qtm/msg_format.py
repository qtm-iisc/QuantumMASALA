...
__all__ = [
    "type_mismatch_msg",
    "type_mismatch_seq_msg",
    "value_mismatch_msg",
    "value_not_in_list_msg",
    "obj_mismatch_msg",
]
...
def type_mismatch_msg(obj_name: str, obj: Any, l_typ: type | str | list[type | str]):
    ...
    for typ in l_typ:
        ...
        if l_typ_str != "":
            typ = "or " + typ
        l_typ_str += typ
    return f"'{obj_name}' must be {l_typ_str}. " f"got '{type(obj)}' instance instead."
...
def type_mismatch_seq_msg(seq_name: str, seq: Sequence[Any], typ: type | str):
    ...
    if isinstance(typ, type):
        typ = f"'{typ}' instances"
    return f"'{seq_name}' must be a sequence of {typ}. " f"got: {typ_seq}."


def value_mismatch_msg(obj_name: str, obj: Any, value: Any):
    assert isinstance(obj_name, str)
    return (
        f"'{obj_name}' must be equal to {value}. "
...
def value_not_in_list_msg(obj_name: str, obj: Any, l_values: Sequence[Any]):
    ...
    return (
        f"'{obj_name}' must be one of the following: "
        f"{str(l_values)[1:, -1]}. "
        f"got {obj_name} = {obj}"
    )


def obj_mismatch_msg(obj1_name: str, obj1: Any, obj2_name: str, obj2: Any):
    assert isinstance(obj1_name, str)
    assert isinstance(obj2_name, str)
    return (
        f"'{obj1_name}' and '{obj2_name}' must refer to the same object. "
