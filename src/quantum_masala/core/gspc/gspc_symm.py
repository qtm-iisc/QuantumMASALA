...
class SymmMod:
    def __init__(self, cryst: Crystal, gspc_cryst: np.ndarray):
        reallat_symm, recilat_symm = get_symmetry_crystal(cryst)
        ...
        ix, iy, iz = gspc_cryst
        self.gspc_size = gspc_cryst.shape[1]
        ...
        l_cryst_rot = np.tensordot(rot_recispc, gspc_cryst, axes=1)
        l_phase_rot = np.exp(
            TPIJ
    ...
    def symmetrize(self, arr_g: np.ndarray):
        ...
        arr_ = arr_g.reshape(-1, self.gspc_size)
