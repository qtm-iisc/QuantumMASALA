...
from ..constants import E_ryd, q_ryd


@dataclass
class UPFv2Data(PseudoPotFile):
    r"""Container to store data from UPF Files v.2.0.1. All quantities are in Hartree Atomic Units.
    ...
    implementation.
    TODO: Read atomic wavefunctions in UPF
    """
    ...
    def read(cls, label: str, dirname: str):
        ...
        for child in root:
            if child.tag == "PP_HEADER":
                ...
                data["vloc"] = np.array(child.text.split(), dtype=np.float64) * E_ryd
        ...
        if data["number_of_proj"] != 0:
            ...
            for gchild in child:
                if gchild.tag == "PP_DIJ":
                    dij = np.array(gchild.text.split(), dtype=np.float64) * E_ryd
